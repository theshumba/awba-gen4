...
| 01-02-T1 | 01-02 | 2 | FND-01 (@layer once) + FND-02 (per-unit 7-slot scales) | T-01-ID | single token source; no shared --blue2/3 | lint (grep) | `grep -c '^@layer [a-z]*, ' shared/awba-engine.css` → 1; 4× `[data-unit]`; `#241A00` present, `#3A2B00` == 0; root-absolute `url(/…)` grep == 0 (file:// safety) | ✅ after task | ⬜ pending |
...
| 01-03-T1 | 01-03 | 3 | FND-02 (recolor switch) + PLT-01 (shell skeleton) | — | N/A | lint + manual | `grep -c 'viewport-fit=cover' preview.html` → 1; `data-unit` switch mutates document element; leading-slash asset-path grep == 0 (file:// safety) | ✅ after task | ⬜ pending |
