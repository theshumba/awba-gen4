...
- [x] **V2-P1 (Wave A): Engine seams + du'a splice** — S1 cream tab-bar rule (real home: @layer screens), S2 AW.S.reset (ringSeed-preserving), S3 streak/noor sheet hoist, S4 AW.practiceRun + single-source PRAISE, G3 default du'a via AW._duaBlock (byte-verified vs CONTENT-DECISIONS), pool extractor + shared/practice-pool.js (32 items) + practice-pool-audit gate; suite 157→177 (completed 2026-07-15, commits b6c2d8f..cac5b7d)
- [x] **V2-P2 (Wave B): The four pages** — onboarding.html (3-pane threshold, empty-Ring hero) / practice.html + practice/session.html (deterministic 8-item sets, zero writes) / profile.html (Ring ledger + on-device name) / more.html (settings, honesty, 2-sheet start-over); 3 built in parallel worktrees + onboarding rebuilt on main after an infra stall (completed 2026-07-15, merges 31caed1/661c132/d18904d + e9ca77f)
- [x] **V2-P3 (Wave C): Integration** — learn.html §0.4 redirect + real tab nav + S3 adoption; sw.js 46→52 precache + cache v2; all 5 gate discovery lists extended honestly (render-smoke 26, rtl 26, contrast 25 pages, glyph +6 sources, a11y-keyboard +5); cream-ghost + .se-prog engine CSS; pf-units + first-run-redirect tests; README; suite 177→184 (completed 2026-07-15, commits 412c530..92254f0)
- [x] **V2-P4: Review + gates + deploy** — 5-lens adversarial review (11 raw → 8 deduped → 3 confirmed Warnings, 0 contested; all 8 judgment calls upheld) → 5 fixes at 4055960 (label-in-name a11y, §D.2 copy, scoped mount motion, double-speak, switch off boundary); full board re-run green by orchestrator (suite 184/0/0 · validate 3-notes · port 20 OK · pool 32 OK · glyph 84 · smoke 26 · rtl 26 · pwa 52 · contrast 25/0); deployed (completed 2026-07-15)

# Milestone v2.1: Fast-follows (completed + deployed 2026-07-15)

Design→build→review workflow (binding spec `.planning/v2-intel/DESIGN-V2.1.md`); review found 0 defects, 3 suggestions all fixed at 8d28b89.
- [x] **S5 — Profile sprout garden** — AW.SPROUTS/AW.sproutFor hoisted from learn.html (SHA-pinned byte-identical, IIFE-sealed privates), learn adopts, Profile gains the finished-lesson garden (shelf grammar, never a second map; "your garden · n of 15 grown"; honest empty state) (346e65f/f1aea15/e6d4060)
- [x] **S6 — course-structure single source** — shared/course-structure.js (window.AWBA_COURSE, byte-copied labels incl. 'One religion, one thread'); learn + profile derive and drop their literals (output-identical, headless-diff proven); precache 53 + cache v3; structure tests (b348fa0/7385102)
- [x] **S7 — scholar review pack** — scripts/build-scholar-pack.js → docs/scholar-pack.html (47 citations, 11 verse beats, 7 daily verses, 1 du'a, holds + sign-off checklist; extraction-only, count-gated generator, excluded from precache/gates) (a540f87)
- [x] **Close** — version 2.1 footer; suite 192/192/0; full board green (smoke 26 · rtl 26 · contrast 25/0 · pwa 53/v3 · glyph 84 · pool 32); deployed

# Milestone v2.2: Take your light with you (completed + deployed 2026-07-16)

Design→build→2-lens adversarial review (data-safety attacker + craft/honesty, skeptic-verified inline; binding spec `.planning/v2-intel/DESIGN-V2.2.md`). Review: 0 Critical / 3 Warnings — all confirmed + fixed inline (SW version-skew guard in openMoveSheet, launch-kit R-8 timer sentence corrected to match the real visible-drain bar, `.mr-code-field` raised to the 44px bar) — plus 2 defensive notes applied (import counter ceiling 9999999, days dedupe+sort) and 2 documented-accepted (YMD shape-tolerance verified benign; the shared-trap Shift+Tab-from-heading quirk predates v2.2, no consent bypass).
*(Preamble, recorded here: R-7 filled 2026-07-16 at 2c40413 — the Ibrahim 14:24 Clear Quran epigraph, owner-authorized, triple-mirror-verified, SHA-pinned in port-audit; pending-review marker stays.)*

- [x] **E1 — Progress travel code** — engine seam S8 `AW.S.exportToken`/`importToken` (`AWBA1.<base64>.<FNV-1a/base36>` grammar; progress-only whitelist so prefs can never leak; ringSeed rides along — the same maker's mark re-inks; REPLACE-never-merge behind a keep-default confirm; stars clamp 1..3 against lessons∪reviews, chests against chest ids, unknown ids dropped + counted; future-schema + memFallback + garbage refusals with gentle whys); More "Move to a new device" row + two-half sheet + arrival confirmation + SW-skew guard; sw.js `awba-cache-v3`→`v4` (engine is cache-first); `state-token.test.js` 10 tests; suite 192→202
- [x] **E2 — Owner launch kit** — `docs/launch-kit.html` (self-contained, print-friendly, not precached, not gate-discovered, own namespaced checklist key): 12-item on-phone device walk with persisted ticks, the awba.app DNS cutover (records corrected from the spec's typo: A 185.199.108–111.**153**), the Clear-Quran-licensing + scholar-gate truth, R-6 Arabic-terms slot shape, the open-decisions ledger
- [x] **E3 — Sound audition kit** — `scripts/build-sound-candidates.js` (zero-dep, deterministic, no clock/entropy) → 9 candidate WAVs (wood/string/thump × correct/incorrect/complete; ≤180/140/350 ms; peak −12 dBFS, incorrect −16 dBFS and lower) + `docs/sound-audition.html`; NOTHING wired — the app stays silent (D-52 owner-gated; shared/sfx holds only its Phase-4 .gitkeep)
- [x] **Close** — version 2.2 footer; suite 202/202/0; full board green (smoke 26 · rtl 26 · contrast 25 pages/0 fail · pwa 53/v4 · glyph 84 · pool 32 · port-audit BYTES+DAILY+EPIGRAPH OK · validate 3 accepted notes); deployed

# Milestone v2.3: Sound & the four terms (completed + deployed 2026-07-16)

Owner directives (voice, 2026-07-16): wire real sound from free sounds online (no music) · fill R-6 with everything includable · close the open product decisions ("do that"). Executed inline; suite 204/204/0; full board green.

- [x] **Sound LIVE (D-52 resolved by owner)** — 4 cues at `shared/sfx/` (correct/incorrect/complete/streak), sourced free + CC0 from the Kenney "Interface Sounds" pack (downloaded, measured, selected against the mercy laws: incorrect = lowest pitch 507Hz + quietest −16 dBFS, never a buzzer; others −12/−13 dBFS; Kenney's own error_* cues REJECTED as buzzer-bright), converted mono 44.1k MP3; `AW.sound` needed ZERO change (the designed seam); sw.js precache 53→57 + cache v4→v5; provenance in README/audition page/engine comment; audition page updated honestly (live cues playable + synth candidates kept for reference)
- [x] **R-6 FILLED (owner-authorized "include everything you can")** — the 4 unit Farag squares now wear verified Arabic chapter-terms via `shared/course-structure.js` term registry: u1 عَقِيدَة · u3 تَوْحِيد · u4 رُبُوبِيَّة (BYTE-VERBATIM from the owner's frozen lesson term entries, pinned by course-structure.test.js) + u2 زَيْغ (sourced Qur'anic vocabulary 3:7/3:8, triple-mirror-verified, exact-string-pinned); learn.html unit headers + continue card render the square with the English title beside/fallback; one `[lang="ar"]` engine-CSS variant (tokens only, the .g-ar.chapter precedent); stale-cache skew degrades to the English fallback by design; CONTENT-DECISIONS G2 addendum records the re-ruling; launch-kit §3 presents all four for scholar confirmation; rtl 6/6 ar-nodes on learn, glyph 84 covered, contrast 0 fail, verified visually at 390px
- [x] **Decisions CLOSED (owner delegated)** — practice-noor: none · R-8: no numeric countdown · practice pool: lessons-only · denominator: 61 · CTAs: calm sub-lines · du'a: Ibn Hibban 974 default; launch-kit §4 records each ruling + why, all reopenable · launch-kit walk grew a 13th item (sound tap-walk)
- [x] **Close** — version 2.3 footer; suite 204/204/0; board green (smoke 26 · rtl 26 · contrast 25/0 (3261+2567) · pwa 57/awba-cache-v5 · glyph 84 · pool 32 · port+validate green); deployed
